...
<script>
    ...
    let undoClickedCount = 0;

    let audio = new Audio("./src/assets/thinking.mp3");
    
    let countSound = new Audio("./src/assets/count.mp3");
    ...
    $: { 
        if (Sum > NumberActive) {        
            countSound.play(); // Start playing the audio
        }else{
            countSound.pause(); // Pause the audio
            countSound.currentTime = 0; // Reset the audio to the beginning
        }
    }
    ...
    function checkFormatUserCount() {
        ...
        if (intValue < 1 || intValue > count || userCount === "" || isNaN(intValue)) {
            if(count<=max-Sum){
                ...
                if(max-Sum!==1){
                    inputError = true;
                    textInputError = `Choose a number between 1 and ${max-Sum}`;
                }else{
                    inputError = true;
                    textInputError = `Choose a number only ${max-Sum}`;
                } 
            }       
        } else {
            if(intValue<=max-Sum){
...
<div class="flex flex-row items-center justify-center h-screen w-screen" style="background-color: #ffe89c;">
    <div class="flex flex-col items-center justify-center">
        <div class="grid-box">
            ...
            <div class="box undo flex items-center justify-center" 
                ...
                class:hoverUndo
                class:disabledUndo={undoClickedCount >= 1 || IsUndoClicked || comTurn || Sum > NumberActive || (round===1&&userTurn) || (round===2&&userTurn) }
                >
    ...
    <BackgroundAnimation maxNumbers={3} speed={20} />
</div>